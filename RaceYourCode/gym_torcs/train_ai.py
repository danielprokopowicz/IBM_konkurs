...
def replay_mode(lap_threshold=2200):
    ...
    for run in range(3):
        env.reset()
        # Naprawiony "rozrusznik" - idealnie płynny start!
        for _ in range(5):
            try:
                d = env.client.S.d
                angle = float(d.get('angle', 0.0))
                tpos = float(d.get('trackPos', 0.0))
                steer_f = float(np.clip(angle * 15.0 / PI - tpos * 0.10, -1.0, 1.0))
                env.step(np.array([steer_f, 0.4, 0.0], dtype=np.float64))
            except Exception:
                pass
...
def rl_phase(actor, critic, env, episodes=1000, lap_threshold=2200):
    ...
    for ep in range(episodes):
        ...
        # Oblicz target_speed PRZED stabilizacją - żeby gaz też był odpowiedni
        target_speed = get_target_speed(ep, lap_count)

        # Stabilizacja z właściwą prędkością docelową (nie hardcoded 0.3)
        for _ in range(5):
            try:
                d     = env.client.S.d
                angle = float(d.get('angle', 0.0))
                tpos  = float(d.get('trackPos', 0.0))
                speed = float(d.get('speedX',  0.0))
                steer_f = float(np.clip(angle * 15.0 / PI - tpos * 0.10, -1, 1))
                accel_f = float(np.clip((target_speed - speed) / max(target_speed, 1), 0.05, 0.4))
                env.step(np.array([steer_f, accel_f, 0.0], dtype=np.float64))
            except Exception:
                pass
        ...
        print(f"  ep {ep:4d} start | target_speed={target_speed:.0f}km/h | "
              f"szum={noise:.3f}", flush=True)
